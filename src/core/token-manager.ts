...
export class TokenManager {
  ...
  private listeners: Map<TokenType, Set<(value: string | undefined) => void>> = new Map();

  constructor(storage?: Storage, expirationConfig?: TokenExpirationConfig) {
    this.storage = storage || (typeof window !== 'undefined' ? window.localStorage : {} as Storage);
    this.expirationConfig = expirationConfig;
    this.loadTokensFromStorage();
  }

  subscribe(tokenType: TokenType, callback: (value: string | undefined) => void): () => void {
  ...
  setToken(type: TokenType, token: string | undefined): void {
